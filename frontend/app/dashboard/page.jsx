    ...
    <div className="min-h-screen max-w-4xl mx-auto px-4 sm:px-6 py-10">
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mt-6 mb-8">
          {[
            { label: 'Questions Asked', value: history.length, icon: MessageSquare, color: 'text-amber-400' },
            { label: 'Saved Laws', value: savedLaws.length, icon: Bookmark, color: 'text-blue-400' },
          ].map((stat) => (
          ...
          {[{ id: 'history', label: 'Query History', icon: Clock }, { id: 'saved', label: 'Saved Laws', icon: Bookmark }].map((tab) => (
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all ${
                activeTab === tab.id ? 'bg-amber-500/20 text-amber-400' : 'text-gray-500 hover:text-gray-300'
              }`}
            >
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {history.length === 0 ? (
              <div className="text-center py-16 text-gray-600">No questions asked yet. <Link href="/ask" className="text-amber-400">Ask one now →</Link></div>
            ) : history.map((item) => (
          ...
          <div className="space-y-3">
            {savedLaws.length === 0 ? (
              <div className="text-center py-16 text-gray-600">No saved laws yet. <Link href="/library" className="text-amber-400">Browse library →</Link></div>
            ) : savedLaws.map((item) => (
              <div key={item.id} className="bg-white/5 border border-white/10 rounded-xl p-4 hover:bg-white/8 transition-all group">
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1 min-w-0">
